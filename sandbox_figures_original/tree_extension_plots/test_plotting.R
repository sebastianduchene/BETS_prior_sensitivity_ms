...
par(mar = c(5, 4, 1, 1))
plot(1, 1,
  type = "n", xlim = c(0, 230), ylim = c(0, 3.2), bty = "n", yaxt = "n", ylab = "Time",
  xaxt = "n", xlab = ""
)
axis(2,
  at = log10(1 + c(0, 0.5, 2, 5, 10, 50, 100, 500, 1000)),
  labels = c(0, 0.5, 2, 5, 10, 50, 100, 500, 1000), las = 2
)
axis(1,
  at = c(25, 75, 135, 200), labels = c(
    "SC\nisochronous\n(True model)", "SC\nheterochronous",
...
ultrametric_sc <- reorder.phylo(
  x = read.nexus("ultrametric_5__SC_iso_gamma.tre"),
  order = "postorder"
)
plot.tree.lines(ultrametric_sc, 3 * pi / 2, 0, 0, log.scale = T)

heterochronous_sc <- reorder.phylo(
  x = read.nexus("ultrametric_5__SC_het_gamma.tre"),
  order = "postorder"
)
plot.tree.lines(heterochronous_sc, 3 * pi / 2, x.offset = 55, 0, log.scale = T)

ultrametric_ucld <- reorder.phylo(
  x = read.nexus("ultrametric_5__UCLD_iso_gamma.tre"),
  order = "postorder"
)
plot.tree.lines(ultrametric_ucld, 3 * pi / 2, x.offset = 110, 0, log.scale = T)

heterochronous_ucld <- reorder.phylo(
  x = read.nexus("ultrametric_5__UCLD_het_gamma.tre"),
  order = "postorder"
)
...
par(mar = c(5, 4, 1, 1))
plot(1, 1,
  type = "n", xlim = c(0, 230), ylim = c(0, 0.8), bty = "n", yaxt = "n", ylab = "Time",
  xaxt = "n", xlab = ""
)
axis(2,
  at = log10(1 + c(0, 0.1, 0.2, 0.4, 0.5, 1, 2, 3, 4)),
...
ultrametric_sc <- reorder.phylo(
  x = read.nexus("heterochronous_5__SC_iso_gamma.tre"),
  order = "postorder"
)
plot.tree.lines(ultrametric_sc, 3 * pi / 2, 0, 0, log.scale = T)

heterochronous_sc <- reorder.phylo(
  x = read.nexus("heterochronous_5__SC_het_gamma.tre"),
  order = "postorder"
)
plot.tree.lines(heterochronous_sc, 3 * pi / 2, x.offset = 55, 0, log.scale = T)

ultrametric_ucld <- reorder.phylo(
  x = read.nexus("heterochronous_5__UCLD_iso_gamma.tre"),
  order = "postorder"
)
plot.tree.lines(ultrametric_ucld, 3 * pi / 2,
  x.offset = 115,
  0, log.scale = T
)

heterochronous_ucld <- reorder.phylo(
  x = read.nexus("heterochronous_5__UCLD_het_gamma.tre"),
  order = "postorder"
)
plot.tree.lines(heterochronous_ucld, 3 * pi / 2, x.offset = 170, 0, log.scale = T)
dev.off()
