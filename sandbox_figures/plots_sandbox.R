...
mle_calculations <- read.table("BETS_empirical_s.csv", head = T, sep = ",")
...
make_plots <- function(dataset){
  ...
  text(c(-10.5, 10.5, 10.5, -10.5), 
       c(11.5, 11.5, -11.5, -11.5), labels = c("Het/SC", "Iso/SC", "Iso/UCLD", "Het/UCLD"), cex = 1)
...
names_dict <- data.frame(c("Vibrio cholerae","Powassan virus","Treponema pallidum"),row.names = datasets)

pdf("polygon_plot.pdf",width=8,height=3.5)
par(mfrow = (c(1,3))) 

make_plots(datasets[1])
make_plots(datasets[2])
legend(-18,-17, legend=c("Exponential","Gamma","Lognormal"), col=c(pal[1],pal[2],pal[3]),pch=c(1,1,1),lty=1,xpd=TRUE,cex=1,pt.cex=1.2, horiz = TRUE,bty = "n",text.width=c(9,6.5,7))
make_plots(datasets[3])
dev.off()
