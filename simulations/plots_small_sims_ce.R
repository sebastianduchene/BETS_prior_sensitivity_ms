...
setwd("~/PhD/prior_sensitivity/figures/")

#mle_calculations <- read.table("~/Documents/Work/Pasteur/BETS/figures/BETS_empirical.csv", head = T, sep = ",")
mle_exp <- read.csv("exponential_marginal_likelihoods_ucld_ucld.csv", row.names = 1, sep = ",")
mle_exp <- mle_exp[, c("SC_het", "SC_iso", "UCLD_iso", "UCLD_het")]
mle_log <- read.csv("lognormal_marginal_likelihoods_ucld_ucld.csv", row.names = 1, sep = ",")
mle_log <- mle_log[, c("SC_het", "SC_iso", "UCLD_iso", "UCLD_het")]
mle_gamma <- read.csv("gamma_marginal_likelihoods_ucld_ucld.csv", row.names = 1, sep = ",")
...
pdf("coalescent_exponential_sims.pdf",width=9,height=7)
par(mfrow = (c(2,3))) 
