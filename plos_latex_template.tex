...
We conducted prior predictive simulations for the six prior configurations for $\theta$ under a heterochronous data set. In Fig \ref{figure:prior_tree_distros} we show the resulting distribution on the root height. The exponential distribution ($\theta \sim Exponential(\mu=1.0)$ produced trees with root heights with a mean of 2.90 units of time (95\% quantile range, qr: 1.62 to 14.34), with corresponding values of 772.11 (qr: 1.66 to $5.06 \times 10^5$) for the log-normal (log-normal$(\mu=1.0, \sigma=5.0)$) and 1.61 (qr: 1.50 to 4.87) for the $\Gamma$ ($\Gamma(\kappa=0.001, \theta=1000)$). Clearly, the log-normal is the most vague prior here, but it produces implausible values. In contrast, the exponential and $\Gamma$ capture our expectation of a tree height of between one and ten units of time (as simulated using $\theta=1.0$.


\textcolor{red}{Talk about whyt the lognromal is more likely to lead to tree extension and the numbers for the bounds}
