...
\begin{flushleft}
{\Large
\textbf\newline{Untangling temporal signal and the phylodynamic threshold of microbial genome data} % Please use "sentence case" for title and headings (capitalize only the first word in a title (or heading), the first word in a subtitle (or subheading), and any proper nouns).
}
\newline
...
The \textit{V. cholerae} data set displayed overwhelming support for temporal signal (Table \ref{table:empirical_bayes_factors}, regardless of the molecular clock model and prior on $\Phi$, with log Bayes factors of over 200. Note that a log Bayes factor of 3.2 corresponds to a model posterior probability $\approx$0.95 \cite{tay2023detecting}, and signifying 'very strong support`, following Kass and Raftery \cite{kass1995bayes}. \textcolor{red}{we need to state the imapct of the clock model and of the prior. If we focus on the clock models, it appears that strict clock in most cases detected temporal signal better, except for the gamma prior. Then the priors have a measurable impact on the Bayes factors, and marginal likelihoods, even if they ultimately did not impact the detection of temporal signal.}
...
\textcolor{red}{John, please add information about the three sequence data sets here. I found some notes we took on site patterns and others:
Site patterns
Empirical
Cholera: 1392
Powassan: 3457
...
\textcolor{red}{John, can you add details on how we ran these, e.g. MCMC length, marginal likelihood settings, subst. model and parameter priors}
