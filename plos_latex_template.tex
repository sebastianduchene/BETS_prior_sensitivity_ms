...
Defining a suitable prior for the parameters of the tree prior requires careful attention. The prior on $\theta$ (or other parameters) should not favour implausible node heights. However, understanding the interplay between the parameters of the tree prior and the resulting tree topologies and node heights is not necessarily trivial, particularly when the tree prior involves multiple parameters (e.g. the Bayesian skyride). A pragmatic solution is to include additional prior information in the form of hard bounds on the root-height or the molecular clock rate. To this end, we investigated the effect of including a uniform prior between 0.0 and 5.0 units of time for the root height, meaning that trees that are older than 5.0 units have a prior probability of 0.0. Importantly, the trees under which we generated our simulations had root heights of around 2.0 units of time, and thus the hard bound of 5.0 allows for trees that are over twice as old as the truth. 
...
We conducted prior predictive simulations for the six prior configurations for $\theta$ under a heterochronous data set. In Fig \ref{figure:prior_tree_distros} we show the resulting distribution on the root height. The exponential distribution ($\theta \sim Exponential(\mu=1.0)$ produced trees with root heights with a mean of 2.90 units of time (95\% quantile range, qr: 1.62 to 14.34), with corresponding values of 772.11 (95\% qr: 1.66 to $5.06 \times 10^5$) for the log-normal (log-normal$(\mu=1.0, \sigma=5.0)$) and 1.61 (95\% qr: 1.50 to 4.87) for the $\Gamma$ ($\Gamma(\kappa=0.001, \theta=1000)$). Clearly, the log-normal is the most vague prior here, but it produces implausible values of several orders of magnitude higher than our expectation of tree heights of around one and ten units of time (as simulated using $\theta=1.0$).
...
Eliciting priors carefully in general appears important for Bayesian model selection (see \cite{bergsten2013bayesian} for a similar problem in topology tests). Our results point to several recommendations to improve tests of temporal signal. First, the parameters of tree prior for isochronous and heterochronous analyses should be in the same units of time, which facilitates comparison of the models in question. For example, fixing the evolutionary rate to 1.0 in an isochronous analysis means that branch lengths are in units of subs/site and thus the coalescent parameters have a different meaning to those estimated under heterochronous analyses (where the evolutionary rate is a free parameter). A more tractable approach is to conduct isochronous analyses by fixing the evolutionary rate to a value in the expected order of magnitude. Biological knowledge, such as the negative correlation between genome size and rates of microbes \cite{sanjuan2012molecular, duchene2016genome}, can be helpful for specifying such values. 

A second important recommendation is to conduct prior predictive simulations, which are essential to understand potential interactions between model parameters and ultimately whether the prior expectation is reasonable with respect to our knowledge about the data. In our study, using hard bounds on the tree height alleviated the problem of tree extension when the prior on $\theta$ favoured very old trees, but in many cases there will exist probability distributions that do not make such strong statements and may produce trees with sensible root heights (e.g. the exponential prior on $\theta$ had low type I and type II errors in our simulations). Finally, prior sensitivity analyses can be useful to determine the extent to which the prior can influence the posterior \cite{foster2017evaluating, lopes2011confronting}, and model selection using Bayes factors \cite{lambert2018student}. A prior that is overly influential on the posterior should be considered carefully, and potentially excluded. The exponential prior on $\theta$ in the \textit{Powassan virus} did not support temporal signal, but note that it is much more influential on the posterior than the $\Gamma$ and log-normal priors on this parameter. In contrast, the \textit{V. cholerae} data seemed robust to the three priors that we used.


\textcolor{red}{what about when there is no temporal signal. here additional information is needed and mention the Treponema data and the case of ancient dna}




\textcolor{red}{there are other tests of temporal signal, but they are also difficult, and mention the phylodynamic threhsold paper and the regressions. Moreover, Bayesian tests can allow us to incorporate important info, like a date uncertainty, or other stuff.}

\textcolor{red}{For hte ending say that This has important implications for what we classify as a measurably evolving population, for the phylodynamic threshold, and ultimately for justifying estimates of evolutionary rates and times based on tip-calibrations only.		}

\textcolor{red}{
\begin{itemize}
	\item[1] \sout{Our study demonstrates that the choice of prior has an impact in model selection, and can result in incorrectly selecting models for the purpose of detecting temporal signal and choosing molecular clocks.}
	\item[2] The tree height and length are particularly important.
		\subitem This has important implications for what we classify as a measurably evolving population, for the phylodynamic threshold, and ultimately for justifying estimates of evolutionary rates and times based on tip-calibrations only.		
		\subitem  It appears that data sets that have very clear temporal signal, with clocklike behaviour, and informative sequence data, the prior is less influential. \sout{However, BETS tends to incorrectly classify data sets with no temporal signal. This occurs due to tree extension and is most prevalent when we consider a strict clock. Tree extension simply occurs to compensate for the impact in the likelihood due to the inclusion of sampling times in truly ultrametric trees, such that the sampling times span a very small portion of the tree height and the tree is effectively ultrametric.}
		\subitem In the absence of internal node calibrations the tree height and tree length are largely determined by the tree prior and the hyperprior on the parameters that govern it. For this reason conducting prior predictive simulations seems essential to determine whether the resulting trees have reasonable root heights, or tend to favour very old trees. A simple way to mitigate this is to include hard bounds, but in our simulations we found that an exponential prior can be sufficient. However, some data sets include very divergent sequences and we may actually do want somewhat old trees, as appears to be the case with the powassan virus data set. In its original analyses several lines of evidence support temporal signal, and here we find that the exponential tree prior did not favour it.  
		\subitem We also find that the relaxed clock can mitigate this problem of incorrect classification, but in empirical analyses it is difficult to make the decision as to whether there is temporal signal when the clock models disagree, so we recommend prior predictive simulations.
	\item[3] What about simpler tests of temporal signal, like the root-to-tip regression, or date randomisation tests? 
		\subitem Well... the root to tip regression is sometimes unclear (cite the BETS paper where we look at inconclusive regressions) and in the date randomisation test what we are testing is whether a particular set of sampling times produce a different estimate than others, so it can be sensitive to the number of dated samples, and may not be very useful for aDNA, where only a small number of samples are ancient. 
		\subitem BETS is still a solid alternative because some data sets require additional information, such as uncertainty in sampling times, monophyletic constraints and root positions. For example, the treponema data analyses did this and obtained very clear temporal signal. Our results for this data set are less conclusive and may indicate that such information is very important. 
	\item[4] Future practice should involve some sort of prior predictive simulations to elicit a prior that does not favour unreasonable node heights and even rates. Importantly, the isochronous analyses should fix the clock rate to a value within the expected order of magnitude, so that the tree prior is comparable between the isochronous and heterochronous models. Choosing a reasonable prior can be difficult for pathogens that are poorly understood. However, our simulations illustrate an extreme case, where our log-normal prior favoured trees that were many orders of magnitude older than our expectation for the simulations. Similarly, prior sensitivity analyses can also be useful, where the robustness of the posterior can be compared for a subset of priors. Our empirical data demonstrate that the most informative data set, \textit{V. cholerae} was generally robust. Similar approaches of comparing prior and posterior root heights have been proposed to assess temporal signal (cite book chapter).
	\item[5] Closing remarks. Temporal signal is important for recently emerging microbes, data sets of ancient DNA, or when the evolutionary history of a pathogen is unknown. Our study provides guidelines for determining this and for improving the reliability of estimates of evolutionary rates and times.
\end{itemize}}
...
; note that in \cite{drummond2002estimating} the notation $\theta$ is used
...
Tests of temporal signal are contingent on priors producing reasonable estimates of key parameters, especially root height. It is difficult to predict the impact of the tree prior and its hyperpriors on the root height, which can lead to improper model selection. Thus, predictive prior simulations are essential when evaluating prior sensitivity. We tested three hyperpriors on $\theta$ to examine the relationship between parameters and the marginal priors. By placing bounds on the root height, we restrict the effect of tree extension. This allows sampling times, which are indistinguishable from noise when the trees are excessively long, to correctly calibrate true heterochronous trees.

\section*{Materials and methods}
\subsection*{Empirical data}
We selected three different datasets to evaluate temporal signal, \textit{V. cholerae} \cite{devault2014second}, \textit{Powassan virus} \cite{vogels2023phylogeographic}, and \textit{T. pallidum} \cite{majander2020ancient}. These varied in their timespans, number of informative sites, and number of sequences. The most sparse dataset was \textit{T. pallidum}, where sampling dates spanned 481 years (1534 to 2016), with 28 sequences each containing 1,500 SNP sites. Among these samples, there were only two in the 1500's and two in the 1700's, with the bulk of the samples with the rest of the samples concentrated in the 1900's to 2000's. There were 319 sequences with 11,193 sites (the complete genome) for \textit{Powassan virus}, spanning 24 years of sampling times (1995 to 2019). Most of the samples were collected after 2010, with only three isolated in 1995. In total, we had 122 \textit{V. cholera} sequences with 1,57 SNP sites across 73 years of sampling (1937 to 2010). There are 1,392 unique site patterns in \textit{V. cholera}, 3,457 in \textit{Powassan virus}, and 840 in \textit{T. pallidum}. %In the simulated data, there are 580-710 (SC) and 640-710 (UCLD) in cholera, and 110-150 in both SC and UCLD for treponema. For the \textit{Powassan virus} SC, there are $\sim$4800 in the isochronous trees and $\sim$14650 in the heterchronous trees.
...
For the \textit{V. cholerae} and \textit{T. pallidum} our data consisted of SNP sites. To account for such ascertainment bias, we specified the number of constant sites of each nucleotide in our subsequent analyses. We sampled the posterior distribution using Markov chain Monte Carlo as implemented in BEAST1.10. The chain length was $10^8$ steps, sampling every $10^3$ to draw a total of $10^4$ samples. The prior for the evolutionary rate of the strict clock was a CTMC-rate reference prior (i.e. a $\Gamma(\alpha=0.5, \beta=T_l$, with $T_l=$ tree length, and mean=$\alpha/\beta$) \cite{ferreira2008bayesian}. For the log-normal distribution of the relaxed clock we also used the CTMC-rate reference prior for the mean rate (known as the ucld.mean in the program) and an exponential prior with mean 0.33 for the standard deviation (the ucld.stdev in the program). For the tree prior we used a constant-size coalescent with population size, $\theta$ with three possible priors, as described in Table \ref{table:prior_distros_on_Phi}. In all cases we used the HKY+$\Gamma_4$ substitution model, with the default priors in BEAST1.10.
...
\label{S2_Fig}
	\begin{center}
		...
		\textbf{Densities of key statistics of \textit{Powassan virus} empirical data under.} The clock rate, root height, tree length, and coefficient of rate variation are shown under three priors on $\theta$, exponential (red), gamma (blue), and lognormal (green).
	\end{center}
\paragraph*{S3 Fig.}
\label{S3_Fig}
	\begin{center}
		\includegraphics[width=13cm]{sandbox_figures/treponema_density_plot.pdf}\newline
		\textbf{Densities of key statistics of \textit{Treponema palladium} empirical data under.} The clock rate, root height, tree length, and coefficient of rate variation are shown under three priors on $\theta$, exponential (red), gamma (blue), and lognormal (green).
	\end{center}
