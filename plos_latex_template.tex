...
\section*{Discussion}
Our study demonstrates that the choice of prior distribution has an impact in Bayesian model selection and that it can mislead tests of temporal signal. In general, data sets with no temporal signal are easily misclassified when the prior favours implausibly old root heights and low evolutionary rates. This is an important problem because incorrectly detecting temporal signal appears to result in a systematic overestimation of the evolutionary timescale and an underestimation of the molecular clock rates. 

We find that tree extension is the most probable reason for the incorrect detection of temporal signal in BETS, because it reduces the sampling window relative to the root height, such that the trees with sampling times are very similar to ultrametrics trees. The phylogenetic likelihood of such overly old trees can be consistent with that from much shallower trees with no sampling times if their evolutionary rate is very low, such that their genetic distance along their branches is comparable.

\textcolor{red}{the clock model. Ideally, the relaxed clock seems to be more robust because it absorbs sampling times as rate variation (we saw higher coefficients of rate variation), instead of making trees really old, but this is tricky because it might not be the best overall model for the data and we want tests of temporal signal to be consistent for among clock models.}

\textcolor{red}{In contrast, data sets with temporal signal are easier to classify because tree extension is not a problem. To minimise incorrect detection of temporal signal we recommend several things. Eliciting priors carefully and in the correct units for ultra and iso, so that these models only differ in the sampling times, second is doing prior predictive simulations and to check the tree height and clock rate, third is doing prior sensitivity. We find that data sets that have strong temporal signal tend to be robust to different priors. This does not need to be exhaustive, but something like what the powwassan virus paper did is good. }
...
\textcolor{red}{what about when there is no temporal signal}
